...
int testPoint(Computer &m, std::vector<__int64> &rom, int x, int y){
    m.relativeBase = 0;
   ...
   //std::cout << "Testing: " << x << ", " << y << " : " << m.out_pin << std::endl;
    return m.out_pin; 
}

bool findCorner(Computer &m, std::vector<__int64> &rom, int s, int &x, int &y ){
    x = 0;
    y = 0;
    // for(int j = 0; j < s; j++){
    //     for(int i = 0; i < s; i++){
    //         if( testPoint(m, rom, i, j) )
    //             std::cout << '#';
    //         else
    //             std::cout << '.';
    //     }
    ...
    while( true ){
        ...
        std::cout << "Line: "  << y << " : " << fx << std::endl;
        //fx is at the first of the row
        x = fx;
        while( testPoint(m, rom, x + s - 1, y) ){
            if( testPoint(m, rom, x, y + s - 1) )
                return true;
            x++;
        }
        y++;
...
int main(){
    ...
    findCorner(m, rom, 100, x, y);
    std::cout << ( (x+1) * 10000 + y) << std::endl;
    //Setup input
    // int points = 0;
    // for( int y = 0; y < 50; y++){
    //     for( int x = 0; x < 50; x++){
    //         points += testPoint(m, rom, x, y);
    //     }
    // }


    //std::cout << testPoint(m, rom, 0, 0) << std::endl;
